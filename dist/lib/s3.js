...
let client = null;
export function getS3BucketAndRegion() {
    const bucket = process.env.S3_BUCKET_NAME ?? process.env.AWS_S3_BUCKET ?? "";
    const region = process.env.AWS_REGION ?? "us-east-1";
    if (!bucket) {
        throw new Error("S3 is not configured. Set S3_BUCKET_NAME (or AWS_S3_BUCKET) and AWS_REGION.");
    }
...
export function registrationObjectPrefix(userId) {
