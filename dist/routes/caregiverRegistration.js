...
import { createPresignedPutUrl, sanitizeFileName, registrationObjectPrefix, } from "../lib/s3.js";
import { getRegistrationDocumentsForUser, saveRegistrationDocuments, } from "../store/caregiverRegistrationStore.js";
const router = Router();
    ...
    try {
        ...
        if (msg.includes("S3 is not configured") || msg.includes("S3_BUCKET")) {
            res.status(503).json({
                ...
                message: "File storage is not configured. Set S3_BUCKET_NAME and AWS_REGION on the API.",
            });
            return;
        }
        throw e;
...
router.post("/documents", async (req, res) => {
    ...
    try {
        await saveRegistrationDocuments(req.userId, docs.map((d) => ({
