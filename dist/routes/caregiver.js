...
import { getApplicationByUserId, submitCaregiverApplication, } from "../store/caregiverApplicationStore.js";
...
router.post("/application", async (req, res) => {
    ...
    const skills = Array.isArray(body.skills)
        ? body.skills.filter((s) => typeof s === "string")
...
router.get("/application/me", async (req, res) => {
    const user = await findUserById(req.userId);
    if (!user?.roles.includes(USER_ROLE.CAREGIVER)) {
        res.status(403).json({ error: "FORBIDDEN", message: "Not a caregiver" });
        return;
...
router.get("/profile-summary", (req, res) => {
    res.json({
        name: "Profile",
        tier: "gold",
        scorePreview: 72,
