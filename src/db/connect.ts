...
export const DEFAULT_MONGODB_URI =
  "mongodb://localhost:27017/?directConnection=true&ssl=false";
...
function resolveMongoUri(raw: string): string {
  ...
  if (out !== raw) {
    console.warn(
      '[care360] Replaced host "mongo" with 127.0.0.1 in MONGODB_URI. ' +
...
function uriHasDatabasePath(uri: string): boolean {
...
function stillHasDockerMongoHost(uri: string): boolean {
  if (uri.startsWith("mongodb+srv://")) return false;
  return (
    /^mongodb:\/\/mongo([:/?]|$)/.test(uri) ||
    /@mongo:/.test(uri) ||
    /,mongo:/.test(uri)
  );
}
...
function redactUri(u: string): string {
  return u.replace(/\/\/([^/@]+)@/, "//***@");
}
...
export async function connectDb(): Promise<void> {
  ...
  if (stillHasDockerMongoHost(uri)) {
    console.error(
      "[care360] FATAL: MONGODB_URI still uses hostname \"mongo\" (Docker-only). " +
        "Use localhost/127.0.0.1 on your machine, or set MONGODB_SKIP_HOST_REWRITE=1 " +
        "only when the API runs inside Docker.",
    );
    throw new Error(
      'MONGODB_URI hostname "mongo" cannot be resolved on this machine',
    );
  }
  ...
  console.log(
    `MongoDB connected${dbNote} directConnection=${String(opts.directConnection)}`,
  );
}
